...
import 'female/AdminGirlsDashboardPage.dart';
import 'female/admin_old_campus_page.dart';
import 'female/admin_new_campus_page.dart';
import 'female/admin_agriculture_page.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _onUserSuccess(String token, Map u) async {
    ...
    final String name = (u['name'] as String?)?.trim().isNotEmpty == true
        ? u['name']
        : (u['reg_number']?.toString() ?? '');
    ...
    switch (role) {
      ...
      // لو عندك مستخدم عام للبنات
      case 'admin_dash_f':
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => const AdminGirlsDashboardPage()),
        );
        break;

      // ✅ الدور الموحد لمسؤولات الكليات البنات
      case 'CollegeAdmin':
        {
          final col = (u['college'] as String?) ?? '';
          late Widget page;
          switch (col) {
            case 'NewCampus':
              page = AdminNewCampusPage(userName: name);
              break;
            case 'OldCampus':
              page = AdminOldCampusPage(userName: name);
              break;
            case 'Agriculture':
              page = AdminAgriculturePage(userName: name);
              break;
            default:
              // احتياط لو ما كان الكوليج معروف
              page = const AdminGirlsDashboardPage();
          }
          Navigator.pushReplacement(
            context,
      ...
      // أدوار البنات القديمة (لو لسه موجودة بقاعدة البيانات)
      case 'NewCampusAdminF':
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => AdminNewCampusPage(userName: name)),
        );
        break;

      case 'OldCampusAdminF':
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => AdminOldCampusPage(userName: name)),
        );
        break;

      case 'AgricultureAdminF':
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (_) => AdminAgriculturePage(userName: name)),
        );
        break;

      default:
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('دور غير معروف')),
        );
    }
  }
