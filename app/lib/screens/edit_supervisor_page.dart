...
class _EditSupervisorPageState extends State<EditSupervisorPage> {
  ...
  Future<void> _init() async {
    final g = await AuthService.genderForAdmin();
    setState(() {
      _adminGender = g;
      // تأكد أن الكلية الحالية ضمن لائحة الدور، وإلا اضبط أول واحدة
      final allowed = (g == 'female') ? _femaleCols : _maleCols;
      if (_college == null || !allowed.contains(_college)) {
        _college = allowed.first;
      }
      _ready = true;
    });
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await dio.put('/supervisors/${widget.data['id']}', data: {
        ...
        // مهم جدًا: بدون هالسطر السيرفر ممكن يغيّرها إلى male افتراضي
