...
class _AzkariPageState extends State<AzkariPage> {
  ...
  void _increment(int idx) {
    if (_counts[idx] < (azkar[idx]['count'] as int)) {
  ...
  Future<void> _showShareOptions(int idx) async {
    ...
    if (choice == 'text') {
      ...
      // ننتظر كامل الإطار حتى يُرسم boundary
  ...
  Future<void> _captureAndShareImage() async {
    try {
      ...
      if (kIsWeb) {
        ...
        final webFile = html.File([blob], 'azkar.png', {'type': 'image/png'});
        final nav = html.window.navigator as dynamic;
        final canShareFiles = nav.canShare != null &&
            nav.canShare({
                  'files': [webFile]
                }) ==
                true;
        if (nav.share != null && canShareFiles) {
          // هذا فقط على متصفحات تدعم Web Share API مع ملفات
          await nav.share({
            'files': [webFile],
            'text': _shareTextForImage
          });
        } else {
          // التنزيل التلقائي كحل بديل على سطح المكتب
        ...
        // هذا سيفتح قائمة المشاركة على أندرويد/آي‌أو‌إس
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // الـ RepaintBoundary الشفاف لضمان الرسم
          Opacity(
            opacity: 0.0,
