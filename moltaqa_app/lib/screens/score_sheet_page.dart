...
class _ScoreSheetPageState extends State<ScoreSheetPage> {
  ...
  // Initialize immediately so it's never uninitialized
  ...
  void initState() {
    super.initState();
    // Attach listener AFTER controller is non-null
  ...
  Future<void> _loadExams() async {
    ...
    try {
      ...
      final parts = all.where(isPart).toList();
      ...
      double? computeAvg(List<Map<String, dynamic>> list) {
        final scores = list.map((e) => e['score']).where((s) => s != null).map((
          s,
        ) {
  ...
  String _fmtScore(dynamic s) {
    if (s == null) return '-';
    final n = num.tryParse(s.toString()) ?? 0;
  ...
  Widget _buildStudentCard() {
    final typeDesc = widget.studentType == 'intensive'
        ? 'تثبيت (حافظ مسبقاً)'
        : 'عادي';
    return Card(
      color: _cardColor,
